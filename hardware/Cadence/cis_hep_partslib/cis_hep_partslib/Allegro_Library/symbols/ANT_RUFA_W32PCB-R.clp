...
(putprop _clp_cinfo (list (_clpAdjustPt -256:-81 _clp_cinfo)	
	(_clpAdjustPt 256:219.17 _clp_cinfo)) 'l_extents)
(putprop _clp_cinfo (_clpAdjustPt '(-780.0 1702.0) _clp_cinfo) 'l_zeropt)
...
_clp_dbid = _clpDBCreatePin("PAD20" _clpAdjustPt(-199:45.5 _clp_cinfo) 	
	_clp_pin_text (180.000 + _clp_cinfo->f_rotation)
	(_clpConvertPt 199:-45.5 _clp_cinfo) 0.000 _clp_pbuf _clp_sym _clpIsLayout() nil)
...
_clp_dbid = _clpDBCreatePin("PAD20" _clpAdjustPt(-26:45.5 _clp_cinfo) 	
	_clp_pin_text (180.000 + _clp_cinfo->f_rotation)
	(_clpConvertPt 26:-45.5 _clp_cinfo) 0.000 _clp_pbuf _clp_sym _clpIsLayout() nil)
if(null(car(_clp_dbid)) then
	_clp_pbuf = cadr(_clp_dbid)
	_clp_dbid = nil
else
...
_clp_dbid = _clpDBCreateSymbolSkeleton('("ANT_RUFA_W32PCB-R" "PACKAGE") _clpAdjustPt('(0.0 0.0) _clp_cinfo)
...
(when (nequal _clp_sym  -1)
 t )
(_clpInitTextOrientation _clp_text_orient "LEFT" nil _clp_cinfo->f_rotation + 0.000 "28" )
_clp_dbid = (_clpDBCreateText "ANT*"
	(_clpAdjustPt -83:96 _clp_cinfo)  _clp_text_orient 
	"REF DES/SILKSCREEN_TOP" _clp_sym)

(_clpInitTextOrientation _clp_text_orient "LEFT" nil _clp_cinfo->f_rotation + 0.000 "28" )
_clp_dbid = (_clpDBCreateText "ANT*"
	(_clpAdjustPt -110:-25 _clp_cinfo)  _clp_text_orient 
	"REF DES/ASSEMBLY_TOP" _clp_sym)

_clp_path  = (_clpPathStart (list (_clpAdjustPt -252:77 _clp_cinfo))
...
_clp_path  = (_clpPathLine _clp_path (_clpMKSConvert 8.000000 _clp_cinfo->t_from_units
	_clp_cinfo->t_to_units) (_clpAdjustPt -252:-77 _clp_cinfo))
_clpPl = list(
	list("CLIP_DRAWING" _clp_clip_prop_value))
